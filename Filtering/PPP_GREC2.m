...
function[X_final,CXX,xsat,fin_results]=PPP_GREC2(XyeniGPS,XyeniGLO,XyeniGAL,XyeniBDS2,CXX,xsat,GPS_matrix,GLO_matrix,GAL_matrix,BDS2_matrix,filter_par,rnxtime,s)
...
if isempty(obs_BDS2) || isempty(obs_GLO) || isempty(obs_GAL)

    if length(xsat.GREC2)>1


        xsat.G=xsat.GREC2(1:(size(xsat.G,1)),1:2+(size(xsat.G,1)));
        XyeniGPS(6:8,:)=[]; %% for GLO, GAL, BDS2 ISB
        AGPS(:,6:8)=[]; GPS_matrix.A=AGPS ; %% for GLO, GAL, BDS2 ISB
        CXX(6:8,:)=[];   CXX(:,6:8)=[]; %% for GLO, GAL, BDS2 ISB

    elseif  length(xsat.GREC2)==1
    ...
    fin_results.order_num(3:8,1)=[0;0;0;0;0;0]; %for GLO, GAL, BDS2 code and phase

    satGPS=obs_GPS(1:(size(obs_GPS,1)/2),2);
    xsat.G=[satGPS,X(6:end,1),(CXX(6:end,6:end))];
    xsat.R=0; xsat.E=0; xsat.C2=0; xsat.GREC2=0;
    ...
    olculer=[obs_GPS;obs_GLO;obs_GAL;obs_BDS2];
    l=[lGPS;lGLO;lGAL;lBDS2];
    A1=[AGPS(:,(1:8)); AGLO(:,(1:8));AGAL(:,(1:8));ABDS2(:,(1:8))];
    A2=[AGPS(:,(9:end)) ; zeros(size(AGLO,1),size(AGPS,2)-8) ;zeros(size(AGAL,1),size(AGPS,2)-8) ;zeros(size(ABDS2,1),size(AGPS,2)-8)];
    A3=[zeros(size(AGPS,1),size(AGLO,2)-8);AGLO(:,(9:end)) ;zeros(size(AGAL,1),size(AGLO,2)-8) ;zeros(size(ABDS2,1),size(AGLO,2)-8)];
    A4=[zeros(size(AGPS,1),size(AGAL,2)-8) ;zeros(size(AGLO,1),size(AGAL,2)-8) ;AGAL(:,(9:end)) ;zeros(size(ABDS2,1),size(AGAL,2)-8)];
    A5=[zeros(size(AGPS,1),size(ABDS2,2)-8) ;zeros(size(AGLO,1),size(ABDS2,2)-8) ;zeros(size(AGAL,1),size(ABDS2,2)-8) ;ABDS2(:,(9:end))];
    ...
    C10=[CGPS zeros(size(CGPS,1),size(CGLO,2))  zeros(size(CGPS,1),size(CGAL,2)) zeros(size(CGPS,1),size(CBDS2,2))];
    C20=[zeros(size(CGLO,1),size(CGPS,2)) CGLO zeros(size(CGLO,1),size(CGAL,2)) zeros(size(CGLO,1),size(CBDS2,2))];
    C30=[zeros(size(CGAL,1),size(CGPS,2))  zeros(size(CGAL,1),size(CGLO,2)) CGAL zeros(size(CGAL,1),size(CBDS2,2))];
    C40=[zeros(size(CBDS2,1),size(CGPS,2))  zeros(size(CBDS2,1),size(CGLO,2)) zeros(size(CBDS2,1),size(CGAL,2)) CBDS2];
    C=[C10;C20;C30;C40];
    ...
    if s==1
        ...
        for p=1:size(olculerkontrol,1)
            ...
            arasatir2=find(olculerkontrol(p,1)==xsat.GREC2(:,1));
            if ~isempty(arasatir2)
                C03(1:8,sayac1)=CXX(1:8,8+sayac);
                C04(sayac1,1:8)=CXX(8+sayac,1:8);
                sayac=sayac+1;
            ...
            for pp=1:size(olculerkontrol,1)
                arasatir3=find(olculerkontrol(pp,1)==xsat.GREC2(:,1));

                if ~isempty(arasatir3)&& ~isempty(arasatir2)
                    C02(p,pp)=xsat.GREC2(arasatir2,2+arasatir3);
    ...
    Xyeni=[XyeniGPS;XyeniGLO(9:end);XyeniGAL(9:end);XyeniBDS2(9:end)];
    ...
    XBDS2=X((size(obs_GPS,1)/2)+(size(obs_GLO,1)/2)+(size(obs_GAL,1)/2)+9:end);





    sats=[satGPS;satGLO+40;satGAL+100;satBDS2+200];
    Xsats=[XGPS;XGLO;XGAL;XBDS2];
    xsat.GREC2=[sats,Xsats,(CXX(9:end,9:end))];
    xsat.G=[satGPS XGPS];
    xsat.R=[satGLO XGLO];
    xsat.E=[satGAL XGAL];
