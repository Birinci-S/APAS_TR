function [clockdata,clocktime ] = read_ClockFile_multi(filename)
...
while ischar(rows)
    t(i).line =rows ; 
    if strcmp(cellstr(t(i).line(66:length(t(i).line))),'END OF HEADER')
        lastHeaderLine=i;
    end
...
for p=1:length(zaman)
    for pp=epochbilgi(p,1): epochbilgi(p+1,1)
        if strcmp((t(pp).line(1:4)),'AS G')
            gps=1;
            columns=cell2mat(textscan(t(pp).line(5:6),'%f'));
            GPS_clk_bias(p,columns)=cell2mat(textscan(t(pp).line(41+5:59+5),'%f'));
        elseif strcmp((t(pp).line(1:4)),'AS R')
            glonass=1;
            columns=cell2mat(textscan(t(pp).line(5:6),'%f'));
            GLONASS_clk_bias(p,columns)=cell2mat(textscan(t(pp).line(41+5:59+5),'%f'));
            
        elseif strcmp((t(pp).line(1:4)),'AS E')
            galileo=1;
            columns=cell2mat(textscan(t(pp).line(5:6),'%f'));
            GAlILEO_clk_bias(p,columns)=cell2mat(textscan(t(pp).line(41+5:59+5),'%f'));
        elseif strcmp((t(pp).line(1:4)),'AS C')
            beidou=1;
            columns=cell2mat(textscan(t(pp).line(5:6),'%f'));
            BEIDOU_clk_bias(p,columns)=cell2mat(textscan(t(pp).line(41+5:59+5),'%f'));

            elseif strcmp((t(pp).line(1:4)),'AS J')
            qzss=1;
            columns=cell2mat(textscan(t(pp).line(5:6),'%f'));
            QZSS_clk_bias(p,columns)=cell2mat(textscan(t(pp).line(41+5:59+5),'%f'));
        end
    end
end
...
clocktime=zeros(length(zaman),1);
 ...
 if qzss==1
 clockdata.QZSS_clkbias=QZSS_clk_bias;
 end
